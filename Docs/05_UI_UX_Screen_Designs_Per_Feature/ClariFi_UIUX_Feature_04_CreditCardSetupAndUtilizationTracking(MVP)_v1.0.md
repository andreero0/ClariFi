...
Here’s the 4th feature.
## Feature 4: Credit Card Setup & Utilization Tracking (MVP)

### Cards List Screen
##### Cards List Empty State
* Illustration of credit cards with plus symbol
* "Add your first credit card" H2 header
* "Track utilization and never miss a payment" subtitle
* "Add Card" primary button with plus icon
* Three benefit points below:
  * "📊 Real-time utilization tracking"
  * "🔔 Statement date reminders"
  * "💡 Optimization recommendations"

##### Cards List Default State
* "Your Cards" H2 header with total utilization percentage
* Card carousel with snap-to-card scrolling:
  * Each card (343dp x 200dp) with gradient background
  * Bank logo, nickname, last 4 digits
  * Animated utilization gauge (circular progress)
  * Current balance and limit shown
  * "Due in X days" or "Statement in X days" badge
* Page dots below carousel
* "Add Another Card" secondary button
* "Utilization Tips" expandable section at bottom

##### Card Swipe Actions State
* Swipe left reveals actions:
  * "Edit" in Clarity Blue background
  * "Remove" in Error red background
* Actions slide in with spring physics
* Haptic feedback on action activation

### Add Card Screen
##### Add Card Form State
* "Add Credit Card" H2 header with back arrow
* Form fields with floating labels:
  * Card nickname (optional) with suggestion
  * Credit limit with currency formatting
  * Statement day (1-31) with number picker
  * Payment due days after statement
  * APR percentage (optional)
* Bank selector dropdown with logos
* Color picker for card visualization:
  * 8 preset gradients in grid
  * Selected shows checkmark overlay
* "Add Card" primary button at bottom

##### Add Card Validation State
* Real-time validation messages:
  * Statement day adjusts for month-end
  * Warning if day doesn't exist in all months
* Error states show inline with shake animation
* Success checkmarks appear as fields validate

### Card Detail Screen
##### Card Detail Overview State
* Full card visualization at top with parallax scroll
* Animated utilization gauge (larger, 120dp):
  * Percentage in center
  * Color changes: green (0-30%), yellow (30-50%), red (50%+)
  * Smooth transition between colors
* Key dates section:
  * Next statement date with countdown
  * Payment due date
  * Days remaining badges
* Quick actions buttons:
  * "Log Payment" primary button
  * "View History" secondary button
* Recent transactions if connected

##### Utilization History View State
* Calendar view of past utilization:
  * Each day shows mini gauge
  * Statement dates marked with badge
  * Touch to see exact percentage
* Trend graph below:
  * 3-month utilization trend line
  * Average line shown dotted
  * Shaded "optimal" zone (0-30%)

### Payment Logging Screen
##### Payment Input State
* "Log Payment" H2 header
* Large number input with currency symbol
* "Pay minimum / Pay full balance / Custom" segmented control
* Date picker for payment date
* "Recurring payment" toggle
* Real-time utilization preview:
  * Current: X% → After payment: Y%
  * Animated gauge showing change
* "Save Payment" primary button

##### Payment Success State
* Checkmark animation with confetti
* "Payment logged!" message
* Updated utilization shown
* "Set up automatic reminders?" prompt

### Multi-Card Overview Screen
##### Utilization Summary State
* "Total Utilization" hero metric with large gauge
* Card list sorted by utilization percentage:
  * Mini gauges on left
  * Card name and limit
  * Current balance
  * Optimization hint if applicable
* "Optimize Payments" AI suggestion card:
  * "Pay $X on Card A to get under 30%"
  * "View Full Plan" link
