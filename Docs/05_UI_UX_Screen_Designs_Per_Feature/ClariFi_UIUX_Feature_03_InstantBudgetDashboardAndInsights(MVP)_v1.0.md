...
Here’s the 3rd feature.
## Feature 3: Instant Budget Dashboard & Insights (MVP)

### Main Dashboard Screen
##### Dashboard Default State
* StatusBar in Clarity Blue with white icons
* Greeting header "Good morning, Sarah" with dynamic time-based message
* Month selector pill centered (e.g., "November 2024") with chevrons
* Three summary cards in horizontal scroll:
  * Income card (Growth Green accent): Amount with small up arrow animation
  * Expenses card (Clarity Blue accent): Amount with category mini-breakdown
  * Savings card (Wisdom Purple accent): Amount and percentage of income
* Each card has subtle gradient background and micro-interaction on tap
* "Spending Insights" section title
* Animated spending chart (custom SVG):
  * Daily spending bars for current month
  * Today highlighted with pulsing dot
  * Smooth height transitions when switching months
...
* Skeleton screens for each component:
  ...
  * Cards show rounded rectangle placeholders
  * Chart shows wave animation placeholder
* Staggered fade-in as data loads (100ms between elements)

##### Dashboard Pull-to-Refresh State
* Custom refresh indicator with ClariFi logo
* Logo rotates and scales during pull
* "Updating..." text appears at threshold
* Haptic feedback at trigger point
* Smooth spring animation back to top

### Spending Chart Detail Screen
##### Chart Interaction State
* Full-screen chart view with enhanced interactions
* Pinch to zoom on time axis
* Vertical swipe to change value scale
* Long press shows detailed tooltip:
  * Date and total amount
  * Category breakdown for that day
  * Comparison to average
* Legend toggles for hiding/showing categories
* "Export Chart" option in top-right menu

##### Chart Customization State
* Bottom sheet for chart options:
...
* Chart morphs between types with smooth transition

### Category Detail Screen
##### Category Spending View State
* Hero section with category icon (48dp) and name
* Current month spending in large text with trend arrow
* Animated progress bar showing budget usage
* "Transaction History" section:
  * Grouped by date
  * Each shows merchant, amount, date
  * Swipe for quick actions
* Floating action button for adding manual transaction
* Monthly trend mini-graph at bottom

##### Budget Setting Modal State
* Modal slides up with spring animation
* "Set [Category] Budget" header
* Large number input with currency symbol
* Slider for visual adjustment (synced with input)
* "Based on your history, we suggest $XXX" AI hint
* Toggle for "Alert me at 80%"
* "Save Budget" primary button

### Insights Feed Screen
##### Insights List State
* Vertical feed of insight cards with different priorities:
  * High priority: Red left border, exclamation icon
  * Medium: Yellow left border, info icon  
  * Low: Blue left border, lightbulb icon
* Each card contains:
  * Insight title (Body Large)
  * Description text
  * Actionable button if applicable
  * "Dismiss" X in corner
* Cards slide in from right on load (staggered)
* Swipe right to dismiss with fade out
* "Mark as Done" changes card to success state

##### Insight Detail Modal State
* Blurred background of dashboard
* Card expands from tap position to center
* Detailed explanation with:
  * Why this matters section
  * Specific recommendations
  * Historical context graph if relevant
  * "Take Action" primary CTA
* Close with X or swipe down

### Export Screen
##### Export Options State
* "Export Your Data" H2 header
* Format selection cards:
  * PDF Report - "Professional summary with charts"
  * CSV Spreadsheet - "Raw data for analysis"
  * JSON - "For developers and power users"
* Date range selector with calendar picker
* Category checkboxes for filtering
* "Generate Export" button
* Recent exports list below

##### Export Generation State
* Progress overlay with stages:
  * "Gathering data..." with spinner
  * "Creating visualizations..." 
  * "Generating PDF..."
* Each stage shows checkmark when done
